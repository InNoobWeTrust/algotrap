...
Producer invariant: binaries own acquisition, periods, profiles, warm-up, signal meaning, gap selection/order, and adaptation. Chartlib calculates, sorts, deduplicates, defaults, and fetches nothing. Telegram must locally provide Structure Directional and I-Ching Original/Transformed/Nuclear before Phase 05.
...
pub struct IchingPane { pub original: ScalarSeries, pub transformed: ScalarSeries, pub nuclear: ScalarSeries }
...
```
Exactly four panes/no collections: Price (candles plus eight listed overlays), Structure `primary/smoothing/directional`, ATR `reversion`, I-Ching `original/transformed/nuclear`. Every slot is required; candle `volume` is the volume slot. Only aligned `ScalarPoint.value: null` is allowed; each series needs one `Some`.
```rust
pub enum DocumentKind { Registry, Dataset, FixedDocument }
pub enum SeriesSlot { PriceVolumeSmoothing, PriceLongTermAverage, PriceBiasReversion, PriceAtrUpperBand, PriceAtrLowerBand, PriceReverseRsiNeutral, PriceReverseRsiBullish, PriceReverseRsiBearish, StructurePrimary, StructureSmoothing, StructureDirectional, AtrReversion, IchingOriginal, IchingTransformed, IchingNuclear }
