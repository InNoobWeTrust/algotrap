...
- Use labels exactly `I-Ching Original`, `I-Ching Transformed`, `I-Ching Nuclear`, with three series with distinct visual semantics: `I-Ching Original` solid `#4FC3F7` (evaluated main energy), `I-Ching Transformed` dashed `rgba(255,183,77,0.55)` (`lineStyle: 2`) for derived prediction, `I-Ching Nuclear` zero-cross `BaselineSeries` with transparent positive/negative fills in the `#CE93D8`/purple hue-family. Add exactly one unified watermark or in-template legend identifying all three labels and their respective series visual signatures.`. Configure/express one watermark or in-template legend that identifies all three labels and their series colors.
